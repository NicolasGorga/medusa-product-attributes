...
import { Container, Heading, Text, toast, DropdownMenu, Button, Badge } from "@medusajs/ui"
...
const AttributeDetailPage = () => {
  ...
  const { data: attribute, isLoading } = useQuery<Attribute>({
    ...
    queryFn: async () => {
      const response = await medusaClient.client.fetch<{ attribute: Attribute }>(`/admin/plugin/attributes/${id}`)
  ...
  return (
    <Container className="divide-y p-0">
        ...
        <div className="flex items-center gap-2">
          <DropdownMenu>
            ...
            <DropdownMenu.Content align="end">
              <DropdownMenu.Item onClick={() => toast.success("Edit clicked!")}>Edit</DropdownMenu.Item>
      ...
      <SectionRow title="Global" value={!attribute.product_categories?.length ? "True" : "False"} />

      {(attribute.possible_values && attribute.possible_values.length > 0) && (
        <SectionRow title="Possible Values" value={
          <>
      ...
      {(attribute.product_categories && attribute.product_categories.length > 0) && (
        <SectionRow title="Product Categories" value={
          <>
